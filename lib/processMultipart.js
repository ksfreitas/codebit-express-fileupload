...
module.exports = function processMultipart(options, req, res, next) {
  ...
  let fileFlushedPromises = [];

  // Build req.files fields
  busboy.on('file', function(fieldname, file, filename, encoding, mime) {
    const tmpFile = tmp.fileSync({postfix: '.upload', keep: false});
    ...
    file.on('end', function() {
      fileFlushedPromises.push(new Promise(function (resolve) {
  ...
  busboy.on('finish', function () {
    Promise.all(fileFlushedPromises).then(function () {
      next();
    }).catch(function (e) {
      next(e);
    });
